...
function Settings({ config, setConfig, testEmail, emailTesting, schedulerState, systemInfo, onOpenWizard, updateState, checkForUpdates, installUpdate, openFolder, dirty, saving, onSave }) {
  ...
  return <div className="page-stack"><div className="page-title-row"><div><span className="eyebrow">Preferences</span><h2>Settings</h2><p>Control backup timing, retention, concurrency, and notifications.</p></div><SaveBar dirty={dirty} saving={saving} onSave={onSave} /></div>
    ...
    <section className="card"><div className="section-heading"><div><span className="eyebrow">Notifications</span><h3>Email reports</h3></div><label className="toggle"><input type="checkbox" checked={email.enabled === true} onChange={(event) => updateEmail({ enabled: event.target.checked })} /><span /></label></div>
      <SelectField label="How reports are sent" value={email.mode === 'company' ? 'company' : 'smtp'} onChange={(event) => updateEmail({ mode: event.target.value })} hint={email.mode === 'company' ? 'Reports are sent by our servers. You never enter a mail password.' : 'Reports are sent using your own mail server.'}>
        <option value="company">Our servers (recommended)</option>
        <option value="smtp">My own mail server</option>
      </SelectField>
      {email.mode === 'company' ? (
        <>
          <div className="form-grid three"><Field label="Service URL" value={email.relay?.controlPlaneUrl || ''} onChange={(event) => updateEmail({ relay: { ...email.relay, controlPlaneUrl: event.target.value.trim() } })} hint="Provided with your licence." /><Field label="Account ID" value={email.relay?.tenantId || ''} onChange={(event) => updateEmail({ relay: { ...email.relay, tenantId: event.target.value.trim() } })} /><Field type="password" label="Licence key" value={email.relay?.licenseKey || ''} placeholder={config._secretStatus?.relayLicenseKey ? 'Stored securely' : 'Enter licence key'} onChange={(event) => updateEmail({ relay: { ...email.relay, licenseKey: event.target.value } })} /></div>
          <div className={cx('credential-note', config._secretStatus?.relayLicenseKey && 'credential-stored')}><span>{config._secretStatus?.relayLicenseKey ? '✓' : '○'}</span>{config._secretStatus?.relayLicenseKey ? 'Licence key is stored in the operating system credential vault.' : 'The licence key will be moved to secure OS storage when saved.'}</div>
        </>
      ) : (
        <>
          <div className="form-grid three"><Field label="SMTP host" value={smtp.host || ''} onChange={(event) => updateSmtp({ host: event.target.value })} /><Field type="number" label="Port" value={smtp.port || 587} onChange={(event) => updateSmtp({ port: Number(event.target.value) })} /><Field label="Account" value={auth.user || ''} onChange={(event) => updateAuth({ user: event.target.value })} /><Field type="password" label="App password" value={auth.pass || ''} placeholder={config._secretStatus?.emailPassword ? 'Stored securely' : 'Enter app password'} onChange={(event) => updateAuth({ pass: event.target.value })} /></div>
          <div className={cx('credential-note', config._secretStatus?.emailPassword && 'credential-stored')}><span>{config._secretStatus?.emailPassword ? '✓' : '○'}</span>{config._secretStatus?.emailPassword ? 'SMTP password is stored in the operating system credential vault.' : 'The password will be moved to secure OS storage when saved.'}</div>
        </>
      )}
      <div className="form-grid"><Field label="Default recipients" value={email.to || ''} onChange={(event) => updateEmail({ to: event.target.value })} hint="Used when a backup source sets no recipients of its own. Separate several with commas." /><Field label="Subject" value={email.subject || ''} onChange={(event) => updateEmail({ subject: event.target.value })} /></div>
      <div className="card-footer"><Button variant="secondary" busy={emailTesting} onClick={testEmail}>Send test email</Button></div></section>
